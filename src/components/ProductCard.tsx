...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link href={`/products/${product.id}/configure`} className="block group">
      <Card className={`relative flex flex-col justify-between overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out rounded-lg h-48 md:h-56 cursor-pointer ${cardBackgroundColor}`}>
        {isSpecialDeals ? (
